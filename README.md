...
[![version](https://img.shields.io/badge/version-1.0.2-blue)](pyproject.toml)
...
- `persona.soul_path` - path to a markdown file shaping opencode's tone/style. Capped at 8192 chars; injected on each task. The persona block is internal only - the prompt forbids leaking any of it into PRs/commits/code.
...
| M4 | Daemon + Discord (live bring-up) + SIGTERM/SIGKILL recovery | `scripts/m4_acceptance.sh` ✅ |
| M5 | Inline reviewer loop + systemd + healthcheck stale + multi-provider validation | `scripts/m5_acceptance.sh` ✅ (6/6 PASS live) |

Current release: **v1.0.2** - see `git tag -l` for history.

## Testing

```bash
.venv/bin/pytest -q                   # 496 tests, ~12s, no external calls
.venv/bin/mypy nocturne               # type check
.venv/bin/ruff check nocturne tests   # lint

# Live acceptance tests (each spawns real opencode subprocesses + creates real PRs)
bash scripts/m1_acceptance.sh         # ~3-5 min, one PR
bash scripts/m2_acceptance.sh         # ~5-10 min, batch
bash scripts/m3_acceptance.sh         # ~5-10 min, park/resume
bash scripts/m4_acceptance.sh         # ~10-15 min, daemon + Discord
bash scripts/m5_acceptance.sh         # ~7-10 min, full 6-test gate
```
