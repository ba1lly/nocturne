...
Usage: flip_to_public.sh [options]
  --owner OWNER           GitHub owner (required; or set GITHUB_OWNER env var)
  --repo REPO             Repo name (default: nocturne; or set GITHUB_REPO env var)
...
if [[ -z "$OWNER" ]]; then
  echo "FAIL: --owner OWNER is required (or set GITHUB_OWNER env var)" >&2
  echo "      e.g. bash scripts/flip_to_public.sh --owner my-github-handle --confirm" >&2
  exit 2
fi
