...
def _step3_models(answers: WizardAnswers, console: Console) -> None:
    ...
    console.print("  [dim]Nocturne uses 'reasoning' for triage + 'report' for run summaries.[/dim]")
    console.print("  [dim]The coding agent is opencode itself; it picks its own model.[/dim]")
...
def _print_summary(answers: WizardAnswers, console: Console) -> None:
    ...
    if answers.coding_model:
        table.add_row("Coding model (opencode override)", f"{answers.provider}/{answers.coding_model}")
...
def _render_config_yaml(a: WizardAnswers) -> str:
    ...
    coding_line = f"  coding: \"{a.provider}/{a.coding_model}\"\n" if a.coding_model else ""
    return (
