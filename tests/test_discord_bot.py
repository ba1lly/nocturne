...
def test_store_add_and_get_discord_message(inmem_store):
    inmem_store.add_discord_message(12345, "owner/sandbox#42")
    assert inmem_store.get_discord_message_task(12345) == "owner/sandbox#42"
...
async def test_send_need_input_persists_mapping(inmem_store, fake_cfg, fake_discord):
    ...
    msg_id = await bot.send_need_input("owner/sandbox#42", 42, "why?")
    assert msg_id == 11111
    # In-memory cache
    assert bot._task_messages[11111] == "owner/sandbox#42"
    # SQLite persistence
    assert inmem_store.get_discord_message_task(11111) == "owner/sandbox#42"
...
async def test_reply_invokes_resume(inmem_store, fake_cfg, fake_discord):
    ...
    from nocturne.discord_bot import NocturneBot
    _fake, client = fake_discord
    inmem_store.add_discord_message(11111, "owner/sandbox#42")
    ...
    assert callback_called == [("owner/sandbox#42", "my answer")]
...
async def test_mapping_persists_across_restart(inmem_store, fake_cfg, fake_discord):
    ...
    _fake, client = fake_discord
    # Persist a mapping
    inmem_store.add_discord_message(22222, "owner/sandbox#7")
    callback_called: list[tuple[str, str]] = []
    ...
    assert callback_called == [("owner/sandbox#7", "after-restart answer")]
...
async def test_cmd_run_allowed_repo_schedules(
    ...
    bot = NocturneBot(fake_cfg, inmem_store, cb)
    interaction = _make_interaction(fake_cfg.discord.mention_user_id)
    await bot._cmd_run(interaction, "owner/sandbox")
    args, _kwargs = interaction.response.send_message.call_args
