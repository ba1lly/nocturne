...
def _make_test_config(
    repo_slug: str = "owner/playground",
...
class TestDryRun:
    ...
    def test_dry_run_skips_push(self, tmp_path: Path) -> None:
        ...
        with patch("nocturne.cli._load_cfg") as mock_load_cfg, \
            ...
            result = runner.invoke(
                app,
                ["run-once", "--repo", "owner/playground", "--issue", "1", "--dry-run"],
            )
...
class TestRunOnceBatch:
    ...
    def test_run_once_without_issue_calls_run_batch(self, tmp_path: Path) -> None:
        ...
        with patch("nocturne.cli._load_cfg") as mock_load_cfg, \
            ...
            result = runner.invoke(app, ["run-once", "--repo", "owner/playground"])
    ...
    def test_run_once_without_issue_dry_run_forwards_flag(self, tmp_path: Path) -> None:
        ...
        with patch("nocturne.cli._load_cfg") as mock_load_cfg, \
            ...
            result = runner.invoke(
                app, ["run-once", "--repo", "owner/playground", "--dry-run"]
            )
    ...
    def test_run_once_with_issue_still_calls_process_task(self, tmp_path: Path) -> None:
        ...
        with patch("nocturne.cli._load_cfg") as mock_load_cfg, \
            ...
            result = runner.invoke(
                app, ["run-once", "--repo", "owner/playground", "--issue", "1"]
            )
...
class TestResume:
    ...
    def test_resume_list_populated(self, tmp_path: Path) -> None:
        ...
        parked_task = ParkedTask(
            id="owner/playground#1",
            status="parked",
            created_at=now,
            updated_at=now,
            repo_slug="owner/playground",
        checkout_path=str(Path(__file__).resolve().parents[1]),
            issue_number=1,
            title="Test issue",
            body="Test body",
        ...
        assert result.exit_code == 0
        assert "owner/playground#1" in result.stdout
    ...
    def test_resume_task_not_found_exits_1(self, tmp_path: Path) -> None:
        ...
        with patch("nocturne.cli._load_cfg") as mock_load_cfg, \
            ...
            result = runner.invoke(app, ["resume", "--task-id", "owner/playground#1", "--answer", "go ahead"])
    ...
    def test_resume_invokes_askflow_with_answer(self, tmp_path: Path) -> None:
        ...
        mock_resume.assert_called_once_with("owner/playground#1", "go ahead", cfg, mock_store)
    ...
    def test_resume_interactive_prompt_when_no_answer(self, tmp_path: Path) -> None:
        ...
        with patch("nocturne.cli._load_cfg") as mock_load_cfg, \
            ...
            result = runner.invoke(app, ["resume", "--task-id", "owner/playground#1"])

        assert result.exit_code == 0
        assert "Resumed task" in result.stdout
        mock_prompt.assert_called_once()
        mock_resume.assert_called_once_with("owner/playground#1", "interactive answer", cfg, mock_store)
